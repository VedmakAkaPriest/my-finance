...
const Circle = ({ index, selectedIndex }) => {
  return (
    <View
      ...
      <Text style={index === selectedIndex ? styles['selectedcircleTitle'] : styles['circleTitle']}>{index}</Text>
    </View>
  );
};
    ...
    <View style={styles.container}>
      {steps.map((step, index) => (
        <View key={index} style={styles.stepContainer}>
          ...
          <Text numberOfLines={MAX_NUMBER_LINES} ellipsizeMode="tail" style={styles.titleCircle}>
            {step.title}
          </Text>
        </View>
      ))}
