...
import outcomeRawData, { categoriesFromOutcome } from 'fakeData/outcome';
...
const MonthView = ({ style, data }) => (
  <FlatList
    ...
    horizontal
    ListHeaderComponent={
      <View style={{ borderWidth: 1, padding: 4, backgroundColor: 'yellow' }}>
        <Text>{'HEADER'}</Text>
      </View>
    }
  />
...
const DayOfMonthView = ({ item: { date } }) => (
  <DataTable.Title
    ...
    {date && date.format('ddd, Do')}
  </DataTable.Title>
);
const DayDataView = ({ item }) => (
  <DataTable.Cell
    numeric
    style={{ borderWidth: 1, padding: 4, width: 75, flex: 0, height: 40 }}>{`${item}`}</DataTable.Cell>
);
const CategoryOfDataView = ({ item }) => (
  <DataTable.Cell
    style={{
      borderWidth: 1,
      padding: 4,
  ...
  return (
    <View style={styles.mainContainerRow}>
      <View style={styles._bordered}>
        ...
        {categories.map(data => (
          <CategoryOfDataView item={data} key={data} />
        ))}
      </View>
      <ScrollView style={styles.container} horizontal>
        <FlatList
          style={[styles._bordered, {}]}
          data={outcomeRawData.map((data, idx) => ({ data, id: `${idx}` }))}
          renderItem={({ item: { data } }) => (
            <View style={styles.mainContainerRow}>
              {data.slice(1).map((date, idx) => (
                <DayDataView item={data[idx + 1]} key={`${idx}`} />
              ))}
            </View>
          )}
          ...
          ListHeaderComponent={
            <View style={styles.mainContainerRow}>
              {days.map(date => (
                <DayOfMonthView item={{ date }} key={date.unix()} />
              ))}
            </View>
          }
        />
